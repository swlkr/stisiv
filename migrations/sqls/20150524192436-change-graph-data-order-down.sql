drop function site_graph_data(int, int);
...
BEGIN
    return query
    select
      to_char(visits.created_at::date, 'MM/DD/YYYY') as day,
    ...
    where
      ...
      visits.created_at > (CURRENT_TIMESTAMP - INTERVAL '30 days')
    group by visits.created_at::date
    order by visits.created_at::date desc;
END;
$$ LANGUAGE plpgsql;
